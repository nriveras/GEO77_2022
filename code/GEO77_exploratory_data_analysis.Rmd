...
data <- as.data.frame(cbind(pH, pH_cacl2))
linearModel <- lm(pH ~ pH_cacl2 , data = data)
summary(linearModel)
plot(linearModel)
```
...
```{r}
rmse(linearModel$fitted.values, pH_cacl2)
```
...
```{r}
boxplot(pH, pH_cacl2, names = c("pH value", "pH in CaCl2 value"), ylab = "pH in water and CaCl2")

```

+   Data to be displayed: pH and Corg
+   Labeling of the x-axis: names = c("pH value", "K content")
+   Labeling of the y-axis: ylab = "pH value and K content".
...
# automatic check if packages are installed
...
rm(np, rp) 						                                		# remove vectors used for load files
